...
description: |
  Instant Postgres databases for local development, demos, prototyping, and test
  environments via pg.new. No account required, expires after 72 hours unless claimed.
  Use when: provisioning a quick database, setting up DATABASE_URL, seeding a database,
  or auto-provisioning Postgres for a project that needs a real database connection.
---
...
The `connection_string` returned is a pooled connection URL. For a direct (non-pooled) connection (e.g. Prisma migrations), remove `-pooler` from the hostname. The CLI writes both pooled and direct URLs automatically.
...
Status transitions: `UNCLAIMED` → `CLAIMING` → `CLAIMED`. After claiming, `connection_string` returns `null`.
...
For Vite projects, `vite-plugin-db` auto-provisions a database on `vite dev` if `DATABASE_URL` is missing. Install with `npm install -D vite-plugin-db`.

## Agent Workflow

### API path

1. **Confirm intent:** If ambiguous, confirm the user wants a temporary, no-signup database. Skip if they explicitly asked for a quick/temporary database.
2. **Provision:** POST to `https://pg.new/api/v1/database` with `{"ref": "agent-skills"}`.
3. **Parse response:** Extract `connection_string`, `claim_url`, and `expires_at`.
4. **Write .env:** Write `DATABASE_URL=<connection_string>` to `.env`. Do not overwrite an existing key without confirmation.
5. **Seed (if needed):**
   ```bash
   psql "$DATABASE_URL" -f seed.sql
   ```
6. **Report:** Where the connection string was written, which key, and the claim URL. Remind: database works now; claim within 72 hours to keep it permanently.
7. **Optional:** Offer a quick connection test (`SELECT 1`).

### CLI path

1. **Check .env:** If `DATABASE_URL` (or chosen key) exists, do not run. Offer remove, `--env`, or `--key` and get confirmation.
2. **Confirm intent:** If ambiguous, confirm temporary no-signup database. Skip if explicit.
3. **Gather options:** Use defaults unless context suggests otherwise.
4. **Run:**
   ```bash
...
6. **Report:** Where written, which key, claim URL, 72-hour reminder.
7. **Optional:** Offer a quick connection test.
...
Claiming is optional. The database works immediately without it. To claim, the user opens the claim URL in a browser and signs in or creates a Neon account.
...
Region cannot be changed for claimable databases.
